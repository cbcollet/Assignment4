...
author: |
  | Casper Collet
  | c.b.collet@students.uu.nl
  | http://www.github.com/cbcollet
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
group1 <- GrowthSW$growth[treat == "0 Revolutions"]
group2 <- GrowthSW$growth[treat == "More than 0 Revolutions"]
...
The p-value represents the probability of obtaining the observed difference in means between the two groups (0 Revolutions and More than 0 Revolutions) or a more extreme difference, assuming that there is no true difference in the population.
...
The variable rgdp60 represents the level of real GDP per capita in 1960. Including this variable in the model allows us to examine the relationship between the initial level of GDP per capita and the subsequent growth rate. It helps us understand if countries with higher or lower GDP per capita in 1960 tend to have different growth rates.
...
summary_table <- models |>
  modelsummary(stars = TRUE, statistics = c("nobs", "rsq"))
...
Based on the provided information, it seems that the variable rgdp60 is consistently significant across Models 3 and 4, suggesting that the initial level of real GDP per capita in 1960 may be a key driver of economic growth. Additionally, Model 4 indicates that the variable tradeshare is also significant, implying that the average trade share of GDP may have a role in explaining economic growth.
...
```{r}
library(kableExtra)
list(model1, model2, model3, model4) |> modelsummary(stars = T, gof_map = c("nobs", "r.squared")) |> row_spec(3, color = "white", background = "red")
